...
<div style="display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-bottom:22px;">
    <div style="background:#fff; border-radius:var(--radius); border:1px solid var(--border); padding:18px 20px;">
        ...
        <div style="font-size:22px; font-weight:700; color:#16a34a;">৳{{ number_format($monthlyCollection) }}</div>
    </div>
    <div style="background:#fff; border-radius:var(--radius); border:1px solid var(--border); padding:18px 20px;">
        ...
        <div style="font-size:22px; font-weight:700; color:#1d4ed8;">৳{{ number_format($todayCollection) }}</div>
    </div>
    <div style="background:#fff; border-radius:var(--radius); border:1px solid var(--border); padding:18px 20px;">
        ...
        <div style="font-size:22px; font-weight:700; color:#dc2626;">৳{{ number_format($totalDue) }}</div>
    </div>
    <div style="background:#fff; border-radius:var(--radius); border:1px solid var(--border); padding:18px 20px;">
        ...
        <div style="font-size:22px; font-weight:700; color:#d97706;">{{ number_format($dueStudentCount) }} জন</div>
    </div>
</div>

<div class="responsive-grid" style="display:grid; grid-template-columns:1fr 360px; gap:20px; align-items:start;">
    <div class="card">
        <div class="card-header" style="padding:16px 20px;">
            <div class="card-title"><i class="bi bi-cash-coin"></i> সাম্প্রতিক সংগ্রহ</div>
            <div style="display:flex; gap:8px;">
                <a href="{{ route('fees.due') }}" class="btn btn-outline btn-sm"><i class="bi bi-exclamation-triangle"></i> বকেয়া</a>
            </div>
        </div>
        <div style="padding:12px 20px; border-bottom:1px solid var(--border);">
            <form method="GET" action="{{ route('fees.index') }}">
                <div class="filter-bar">
                    <input type="text" name="search" class="form-control" placeholder="ছাত্রের নাম/রশিদ..." value="{{ request('search') }}" style="min-width:200px;">
        ...
        <div class="table-wrap">
            <table class="data-table">
                <thead><tr><th>রশিদ নং</th><th>ছাত্র</th><th>ফি ধরন</th><th style="text-align:right">পরিশোধ</th><th>বকেয়া</th><th>তারিখ</th><th></th></tr></thead>
                <tbody>
                    @forelse($collections as $fee)
                    <tr>
                        <td style="font-family:var(--font-en); font-size:12px; font-weight:600; color:var(--green);">{{ $fee->receipt_no }}</td>
                        <td>
                        ...
                        <td>
                            <a href="{{ route('fees.receipt',$fee->receipt_no) }}" class="btn btn-icon btn-outline btn-sm" target="_blank" title="রশিদ"><i class="bi bi-printer"></i></a>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="7"><div class="empty-state"><i class="bi bi-cash"></i><p>কোনো রেকর্ড নেই</p></div></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="card" style="position:sticky; top:84px;">
        <div class="card-header"><div class="card-title"><i class="bi bi-plus-circle-fill"></i> ফি গ্রহণ করুন</div></div>
        <div class="card-body">
            <form method="POST" action="{{ route('fees.collect') }}">
                @csrf
                <div style="margin-bottom:14px;">
                    <label class="form-label">ছাত্র খুঁজুন <span style="color:red">*</span></label>
                    <div style="position:relative;">
                        <input type="text" id="studentSearch" class="form-control" placeholder="নাম বা আইডি টাইপ করুন..." autocomplete="off">
                        <div id="studentDropdown" style="position:absolute; top:100%; left:0; right:0; background:#fff; border:1.5px solid var(--border); border-radius:var(--radius-sm); max-height:200px; overflow-y:auto; z-index:100; display:none; box-shadow:var(--shadow);"></div>
                    </div>
                    <input type="hidden" name="student_id" id="selectedStudentId" required>
                    <div id="selectedStudentInfo" style="margin-top:8px; display:none; padding:8px 12px; background:var(--green-light); border-radius:var(--radius-sm); border-left:3px solid var(--green);"></div>
                </div>
                <div style="margin-bottom:14px;">
                    ...
                    <select name="fee_category_id" class="form-select" required id="feeCategory">
                        ...
                        <option value="{{ $cat->id }}" data-amount="{{ $cat->amount }}">{{ $cat->name }} (৳{{ number_format($cat->amount) }})</option>
                        @endforeach
                    </select>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:14px;">
                    ...
                    <div>
                        ...
                        <input type="number" id="feeAmount" class="form-control" readonly style="background:var(--bg);">
                    </div>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:14px;">
                    <div>
                        <label class="form-label">ছাড়</label>
                        <input type="number" name="discount" class="form-control" value="0" min="0" id="discount" onchange="calcDue()">
                    </div>
                    <div>
                        <label class="form-label">পরিশোধ <span style="color:red">*</span></label>
                        <input type="number" name="paid_amount" class="form-control" required min="0" id="paidAmount" onchange="calcDue()" placeholder="৳">
                    </div>
                </div>
                <div id="duePreview" style="padding:8px 12px; background:#fef3c7; border-radius:var(--radius-sm); border-left:3px solid #f59e0b; margin-bottom:14px; display:none;">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <input type="text" name="transaction_id" class="form-control" placeholder="ঐচ্ছিক">
                </div>
                <button type="submit" class="btn btn-primary" style="width:100%; justify-content:center;">
                    <i class="bi bi-check-circle-fill"></i> ফি গ্রহণ ও রশিদ প্রিন্ট
                </button>
            </form>
        </div>
    </div>
...
function calcDue() {
    ...
    if (due > 0) { preview.style.display='block'; document.getElementById('dueAmount').textContent='৳'+due.toLocaleString(); }
...
function selectStudent(id, name, sid, cls) {
    ...
    info.innerHTML = `<div style="font-size:13px; font-weight:600; color:var(--green);">${name}</div><div style="font-size:12px; color:var(--text-muted);">${sid} · ${cls}</div>`;
}
